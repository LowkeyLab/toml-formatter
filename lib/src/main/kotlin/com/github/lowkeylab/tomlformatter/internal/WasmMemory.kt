...
internal data class WasmBuffer(
    val pointer: Int,
    val length: Int,
)
...
internal fun WasmFormatterRuntime.allocate(length: Int): WasmBuffer {
    if (length < 0) {
        raise.raise(TomlFormatterError.WasmMemoryFailure("allocate", "length must be non-negative: $length"))
    }
...
context(raise: Raise<TomlFormatterError>)
internal fun <T> WasmFormatterRuntime.withDeallocated(
    buffer: WasmBuffer,
    block: () -> T,
): T {
...
internal fun WasmFormatterRuntime.deallocateFailure(buffer: WasmBuffer): TomlFormatterError.WasmMemoryFailure? =
...
private class WasmCleanupFailure(
    failure: TomlFormatterError.WasmMemoryFailure,
...
internal fun WasmFormatterRuntime.writeBuffer(buffer: WasmBuffer, bytes: ByteArray) {
    if (bytes.size != buffer.length) {
        raise.raise(TomlFormatterError.WasmMemoryFailure("write", "byte count ${bytes.size} did not match buffer length ${buffer.length}"))
    }
...
internal fun WasmFormatterRuntime.invokeSingle(operation: String, vararg args: Long): Long {
    val function = when (operation) {
        ...
        else -> raise.raise(TomlFormatterError.WasmInvocationFailure(operation, "unknown exported function"))
    }

    val results = runCatching { function.apply(*args) }
        .getOrElse { error ->
            raise.raise(TomlFormatterError.WasmInvocationFailure(operation, error.describe()))
        }

    return results.singleOrNull()
        ?: raise.raise(TomlFormatterError.WasmInvocationFailure(operation, "expected one result, got ${results.size}"))
}

context(raise: Raise<TomlFormatterError>)
private fun Long.toIntPointer(operation: String): Int {
    if (this < 0 || this > Int.MAX_VALUE) {
        raise.raise(TomlFormatterError.WasmMemoryFailure(operation, "pointer does not fit Chicory int address: $this"))
    }
    return toInt()
}
...
internal fun ensureRange(memory: Memory, buffer: WasmBuffer, operation: String) {
    if (buffer.pointer < 0 || buffer.length < 0) {
        raise.raise(TomlFormatterError.WasmMemoryFailure(operation, "negative pointer or length: $buffer"))
    }

    val end = buffer.pointer.toLong() + buffer.length.toLong()
    val memorySize = memory.pages().toLong() * Memory.PAGE_SIZE.toLong()
    if (end > memorySize) {
        raise.raise(TomlFormatterError.WasmMemoryFailure(operation, "range $buffer exceeds memory size $memorySize"))
    }
}
