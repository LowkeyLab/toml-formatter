package io.github.lowkeylab.tomlformatter

import arrow.core.raise.either
import io.github.lowkeylab.tomlformatter.internal.decodeProtobuf
import io.github.lowkeylab.tomlformatter.internal.mapFormatterResult
import io.kotest.assertions.arrow.core.shouldBeLeft
import io.kotest.assertions.arrow.core.shouldBeRight
import io.kotest.core.spec.style.FunSpec
import io.kotest.matchers.shouldBe
import io.kotest.matchers.types.shouldBeInstanceOf
import taplo_wasm.FormatToml.FormatTomlFailure
import taplo_wasm.FormatToml.FormatTomlResult
import taplo_wasm.FormatToml.FormatTomlSuccess

class ProtobufResultTest :
    FunSpec({
        test("decodes protobuf bytes") {
            val bytes =
                FormatTomlResult.newBuilder()
                    .setSuccess(FormatTomlSuccess.newBuilder().setFormatted("formatted"))
                    .build()
                    .toByteArray()

            val result = either<TomlFormatterError, FormatTomlResult> { decodeProtobuf(bytes) }

            result.shouldBeRight().success.formatted shouldBe "formatted"
        }

        test("raises protobuf decode errors through Arrow Raise") {
            val result =
                either<TomlFormatterError, FormatTomlResult> {
                    decodeProtobuf(byteArrayOf(0xff.toByte()))
                }

            result.shouldBeLeft().shouldBeInstanceOf<TomlFormatterError.ProtobufDecodeFailure>()
        }

        test("maps formatter success protobuf results") {
            val protobuf =
                FormatTomlResult.newBuilder()
                    .setSuccess(FormatTomlSuccess.newBuilder().setFormatted("formatted"))
                    .build()

            val result = either<TomlFormatterError, String> { mapFormatterResult(protobuf) }

            result.shouldBeRight() shouldBe "formatted"
        }

        test("maps formatter core protobuf failures to FormatterCoreFailure") {
            val message = "taplo could not format this input"
            val protobuf =
                FormatTomlResult.newBuilder()
                    .setFailure(FormatTomlFailure.newBuilder().setMessage(message))
                    .build()

            val result = either<TomlFormatterError, String> { mapFormatterResult(protobuf) }

            result.shouldBeLeft() shouldBe TomlFormatterError.FormatterCoreFailure(message)
        }

        test("raises missing result for a protobuf message without a success or failure") {
            val result =
                either<TomlFormatterError, String> {
                    mapFormatterResult(FormatTomlResult.getDefaultInstance())
                }

            result.shouldBeLeft() shouldBe TomlFormatterError.MissingProtobufResult
        }
    })
